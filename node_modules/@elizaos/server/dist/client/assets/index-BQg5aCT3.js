...
function requireColorette () {
	...
	const {
	  ...
	  argv = [],
	  platform = "",
	} = typeof distExports.process === "undefined" ? {} : distExports.process;
...
function requireEndOfStream () {
	...
	var qnt = globalThis.Bare ? queueMicrotask : distExports.process.nextTick.bind(distExports.process);
...
function requirePump () {
	...
	var noop = function () {};
	var ancient = typeof distExports.process === 'undefined' ? false : /^v?\.0/.test(distExports.process.version);
...
function requirePinoAbstractTransport () {
	...
	function defaultClose (err, cb) {
	  distExports.process.nextTick(cb, err);
	}
	...
	function createErrDiff(actual, expected, operator) {
	  ...
	  if (actualLines.length === 1 && expectedLines.length === 1 && actualLines[0] !== expectedLines[0]) {
	    ...
	    if (inputLength <= kMaxShortLength) {
	      ...
	      var maxLength = distExports.process.stderr && distExports.process.stderr.isTTY ? distExports.process.stderr.columns : 80;
	...
	var AssertionError = /*#__PURE__*/function (_Error, _inspect$custom) {
	  ...
	  function AssertionError(options) {
	    ...
	    if (message != null) {
	      ...
	      if (distExports.process.stderr && distExports.process.stderr.isTTY) {
	        // Reset on each call to make sure we handle dynamically set environment
	        // variables correct.
	        if (distExports.process.stderr && distExports.process.stderr.getColorDepth && distExports.process.stderr.getColorDepth() !== 1) {
	          blue = "\x1B[34m";
	          green = "\x1B[32m";
	          white = "\x1B[39m";
	          red = "\x1B[31m";
...
function requireAssert () {
	...
	function fail(actual, expected, message, operator, stackStartFn) {
	  ...
	  if (argsLen === 0) {
	    ...
	    if (warned === false) {
	      warned = true;
	      var warn = distExports.process.emitWarning ? distExports.process.emitWarning : console.warn.bind(console);
...
function requireSonicBoom () {
	...
	const kEmptyBuffer = distExports.Buffer.allocUnsafe(0);
	...
	const [major, minor] = (distExports.process.versions.node || '0.0').split('.').map(Number);
	...
	function openFile (file, sonic) {
	  ...
	  function fileOpened (err, fd) {
	    ...
	    if (sonic.sync) {
	      distExports.process.nextTick(() => sonic.emit('ready'));
	    } else {
	      sonic.emit('ready');
	    }
	    ...
	    if ((!sonic._writing && sonic._len > sonic.minLength) || sonic._flushPending) {
	      ...
	      distExports.process.nextTick(() => sonic.emit('drain'));
	    }
	  }
	...
	function SonicBoom (opts) {
	  ...
	  if (typeof fd === 'number') {
	    this.fd = fd;
	    distExports.process.nextTick(() => this.emit('ready'));
	    ...
	    if (this._reopening) {
	      ...
	      if (this.sync) {
	        if (!this._asyncDrainScheduled) {
	          ...
	          distExports.process.nextTick(emitDrain, this);
	        }
	      } else {
	        this.emit('drain');
	      }
	...
	function releaseWritingBuf (writingBuf, len, n) {
	  ...
	  if (typeof writingBuf === 'string' && distExports.Buffer.byteLength(writingBuf) !== n) {
	    // Since the fs.write callback parameter `n` means how many bytes the passed of string
	    // We calculate the original string length for avoiding the multi-byte character issue
	    n = distExports.Buffer.from(writingBuf).subarray(0, n).toString().length;
	  }
	...
	function mergeBuf (bufs, len) {
	  ...
	  return distExports.Buffer.concat(bufs, len)
	}
	...
	function actualWriteBuffer () {
	  ...
	  if (this.sync) {
	    ...
	    if (kCopyBuffer) {
	      this._writingBuf = distExports.Buffer.from(this._writingBuf);
	    }
...
function requireOnExitLeakFree () {
	...
	function install (event) {
	  ...
	  distExports.process.on(event, functions[event]);
	}
	...
	function uninstall (event) {
	  ...
	  distExports.process.removeListener(event, functions[event]);
...
function requireSecureJsonParse () {
	...
	const hasBuffer = typeof distExports.Buffer !== 'undefined';
	...
	function _parse (text, reviver, options) {
	  ...
	  if (hasBuffer && distExports.Buffer.isBuffer(text)) {
	    text = text.toString();
	  }
...
function requirePinoPretty () {
	...
	const defaultOptions = {
	  ...
	  outputStream: distExports.process.stdout,
	  singleLine: false,
...
export { index$1 as i };
//# sourceMappingURL=index-BQg5aCT3.js.map
