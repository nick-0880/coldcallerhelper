...
var PluginCreator = class {
  ...
  async getCommaSeparatedInput(message, required = false) {
    ...
    return inputStr.split(",").map((item) => item.trim()).filter((item) => item);
  }
  anthropic = null;
...
`);
  ...
  async collectPluginSpecification() {
    ...
    const description = await clack.text({
      ...
      validate: (input) => input.length > 0 ? void 0 : "Description is required"
    });
