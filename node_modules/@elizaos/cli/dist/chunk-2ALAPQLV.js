...
async function runCommandWithSpinner(command, args, options) {
  ...
  try {
    ...
    if (result.success) {
      ...
      spinner3.stop(colors.red(errorMessage));
...
async function runBunWithSpinner(args, cwd, options) {
...
async function installDependenciesWithSpinner(targetDir) {
...
async function buildProjectWithSpinner(targetDir, isPlugin = false) {
...
async function installPluginWithSpinner(pluginName, targetDir, purpose = "") {
...
async function runTasks(tasks2) {
  await clack.tasks(tasks2);
}
...
async function confirmAction(message) {
  const response = await clack2.confirm({
    message,
    initialValue: false
  });
...
async function storePostgresUrl(url, envFilePath) {
  if (!url) return;
  try {
    const envDir = path9.dirname(envFilePath);
    await fs7.mkdir(envDir, { recursive: true });
    let content = "";
...
async function promptAndStorePostgresUrl(envFilePath) {
  ...
  spinner3.start("Saving PostgreSQL configuration...");
  try {
    await storePostgresUrl(response, envFilePath);
    spinner3.stop("PostgreSQL configuration saved successfully!");
    clack3.outro("\u2713 Database connection configured");
    return response;
  } catch (error) {
    spinner3.stop("Failed to save configuration");
...
async function promptAndStoreProviderConfig(config, envFilePath) {
  ...
  const spinner3 = clack3.spinner();
  spinner3.start(`Saving ${config.name} configuration...`);
  try {
    await config.storeFunction(results, envFilePath);
    spinner3.stop(`${config.name} configuration saved successfully!`);
...
function handleError(error) {
  ...
  if (isNoSpace) {
    ...
    if (error instanceof Error) {
      logger13.error(colors3.red(error.message));
      logger13.error(colors3.red(error.stack || ""));
    } else {
      logger13.error(colors3.red(String(error)));
    }
  } else {
...
function logHeader(title) {
  ...
  const topBorder = colors4.green(`\u250C${"\u2500".repeat(borderLength)}\u2510`);
  const bottomBorder = colors4.green(`\u2514${"\u2500".repeat(borderLength)}\u2518`);
  const coloredTitle = `${" ".repeat(padding)}=== ${colors4.green(title)} ===${" ".repeat(padding)}`;
  const middleRow = colors4.green("\u2502") + coloredTitle + colors4.green("\u2502");
  console.log(`
${topBorder}
${middleRow}
${bottomBorder}`);
...
function getGlobalNodeModulesPath() {
  const execDir = path15.dirname(process.execPath);
  const isBun = typeof Bun !== "undefined";
  if (process.platform === "win32") {
    if (isBun) {
      const bunPaths = [path15.join(execDir, "node_modules")];
      if (process.env.APPDATA) {
        bunPaths.push(path15.join(process.env.APPDATA, "bun", "node_modules"));
      }
      if (process.env.LOCALAPPDATA) {
        bunPaths.push(path15.join(process.env.LOCALAPPDATA, "bun", "node_modules"));
      }
      if (process.env.USERPROFILE) {
        bunPaths.push(path15.join(process.env.USERPROFILE, ".bun", "node_modules"));
      }
      for (const potentialPath of bunPaths) {
        if (existsSync12(potentialPath)) {
          return potentialPath;
        }
      }
    }
    return path15.join(execDir, "node_modules");
  } else {
    if (isBun) {
      const bunUnixPaths = [
        path15.join(execDir, "node_modules"),
        path15.join(execDir, "..", "lib", "node_modules")
      ];
      if (process.env.HOME) {
        bunUnixPaths.unshift(path15.join(process.env.HOME, ".bun", "node_modules"));
      }
    ...
    return path15.join(execDir, "..", "lib", "node_modules");
  }
}
...
function normalizeImportPath(importPath) {
  const normalized = path15.normalize(importPath);
  if (process.platform === "win32" && path15.isAbsolute(normalized)) {
    ...
    try {
      ...
      for (const testFile of testFiles) {
        try {
          ...
          let testModule;
          try {
            testModule = await import(pathToFileURL2(moduleImportPath).href);
          } catch (importError) {
